...
echo "`date +%F" "%R:%S` Fetching remote digest... "
...
if [ "$digest_python" != "$local_digest_python" ] ; then
        echo "`date +%F" "%R:%S` Pulling latest version $IMAGE_PYTHON..."
        ...
        echo "`date +%F" "%R:%S` Pulling latest version $IMAGE_NODEJS..."
        ...
        echo "`date +%F" "%R:%S` Already up to date. Nothing to do."
fi